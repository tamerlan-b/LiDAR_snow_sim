...
def my_process_single_channel(particles: dict, orig_pc: np.ndarray, beam_divergence: float,
                           order: List[int], channel_infos: List, channel: int) -> Tuple:
    """
    :param particles:               N-by-3 array containing sampled particles are stored (x, y, r).
    :param orig_pc:                 N-by-5 array containing original pointcloud (x, y, z, intensity, channel).
    :param beam_divergence:         Equivalent to the total beam opening angle (in degree).
    :param order:                   Order of the particle disks.
    :param channel_infos            List of Dicts containing sensor calibration info.
    ...
    center_angles = np.arctan2(y, x)  # in range [-PI, PI]
    center_angles[center_angles < 0] = center_angles[center_angles < 0] + DOUBLE_PI  # in range [0, 2*PI]

    beam_angles = -np.ones((N, 2))

    beam_angles[:, 0] = center_angles - np.radians(beam_divergence * 0.5)  # could lead to negative values
    beam_angles[:, 1] = center_angles + np.radians(beam_divergence * 0.5)  # could lead to values above 2*PI

    # put beam_angles back in range [0, 2*PI]
    beam_angles[beam_angles < 0] = beam_angles[beam_angles < 0] + DOUBLE_PI
    beam_angles[beam_angles > DOUBLE_PI] = beam_angles[beam_angles > DOUBLE_PI] - DOUBLE_PI
    
    if not index in particles:
        return intensity_diff_sum, idx, pc

    channel_particles = particles[index]

    occlusion_list = my_get_occlusions(beam_angles=beam_angles, ranges_orig=distance, beam_divergence=beam_divergence,
                                    all_particles=channel_particles)

    lidar_range = 120                       # in meter
    intervals_per_meter = 10                # => 10cm discretization
    inv_intervals_per_meter = 1 / intervals_per_meter
    beta_0 = 1 * 10 ** -6 / PI
    tau_h = 1e-8                            #  value 10ns taken from HDL64-S1 specsheet
    ...
    for j, beam_dict in enumerate(occlusion_list):
        ...
        if len(beam_dict.keys()) > 1:                           # otherwise there is no snowflake in the current beam
            ...
            max_index = np.argmax(i)
            i_max = i[max_index]
            d_max = (max_index * inv_intervals_per_meter) - (c * tau_h * 0.5)

            i_max += max_intensity * focal_slope * np.abs(focal_offset - (1 - d_max/120)**2)
            i_max = np.clip(i_max, min_intensity, max_intensity)

            if abs(d_max - d_orig) < 2 * (1 * inv_intervals_per_meter):  # only alter intensity
...
def compute_occlusion_dict(beam_angles: Tuple[float, float], intervals: np.ndarray,
    ...
    inv_beam_divergence_rad = 1. / np.radians(beam_divergence)
    ...
    for j in range(N):
        ...
        if assignment_made:
            ratio = diffs[assignment == j].sum() * inv_beam_divergence_rad
            occlusion_dict[j] = (distance, np.clip(ratio, 0, 1))

    ratio = diffs[assignment == -1].sum() * inv_beam_divergence_rad
...
def my_get_occlusions(beam_angles: np.ndarray, ranges_orig: np.ndarray, all_particles: np.ndarray,
                   beam_divergence: float) -> List:
    """
    :param beam_angles:         M-by-2 array of beam endpoint angles, where for each row, the value in the first column
                                is lower than the value in the second column.
    :param ranges_orig:         M-by-1 array of original ranges corresponding to beams (in m).
    :param all_particles:               N-by-3 array containing sampled particles are stored (x, y, r).
    :param beam_divergence:     Equivalent to the opening angle of an individual LiDAR beam (in degree).
    ...
    for i in range(M):
        ...
        particle_angles = np.arctan2(y, x)                                                                      # (K,)
        particle_angles[particle_angles < 0] = particle_angles[particle_angles < 0] + DOUBLE_PI
        ...
        # which is first sufficient condition for intersection.
        standard_case = np.logical_and(right_angle <= particle_angles, particle_angles <= left_angle)
        seldom_case = np.logical_and.reduce((right_angle - DOUBLE_PI <= particle_angles, particle_angles <= left_angle,
                                             np.full_like(particle_angles, right_angle > left_angle, dtype=bool)))
        seldom_case_2 = np.logical_and.reduce((right_angle <= particle_angles, particle_angles <= left_angle + DOUBLE_PI,
                                               np.full_like(particle_angles, right_angle > left_angle, dtype=bool)))
...
def my_augment(pc: np.ndarray, particles: dict, sensor_info: dict, beam_divergence: float, shuffle: bool = True,
            show_progressbar: bool=False, noise_floor: float=0.7,
            root_path: str=None) -> Tuple:
    """
    :param pc:                      N-by-5 array containing original pointcloud (x, y, z, intensity, channel).
    :param particles:               N-by-3 array containing sampled particles are stored (x, y, r).
    :param sensor_info:             TODO: add description
    :param beam_divergence:         Beam divergence in degrees.
    :param shuffle:                 Flag if order of sampled snowflakes should be shuffled.
    :param show_progressbar:        Flag if tqdm should display a progessbar.
    :param noise_floor:             Noise floor threshold.
    ...
    w, h = calculate_plane(pc)
    pc_matmul = np.matmul(pc[:, :3], np.asarray(w)) + h
    ground = np.logical_and(pc_matmul < 0.5, pc_matmul > -0.5)
    pc_ground = pc[ground]

    calculated_indicent_angle_cos = np.divide(np.matmul(pc_ground[:, :3], np.asarray(w)),
                                                    np.linalg.norm(pc_ground[:, :3], axis=1) * np.linalg.norm(w))

    calculated_indicent_angle = np.arccos(calculated_indicent_angle_cos)
    relative_output_intensity, adaptive_noise_threshold, _, _ = estimate_laser_parameters(pc_ground,
                                                                                          calculated_indicent_angle,
                                                                                          noise_floor=noise_floor,
                                                                                          debug=False)

    adaptive_noise_threshold *= calculated_indicent_angle_cos
    ...
    channel_infos = sensor_info['lasers']
    num_channels = sensor_info['num_lasers']

    channels = range(num_channels)
    order = list(range(num_channels))

    # print(f"channels: {channels}, order: {order}")
    ...
    if show_progressbar:
        channel_list[:] = process_map(functools.partial(my_process_single_channel, particles,
                                                        orig_pc, beam_divergence, order, channel_infos),
                                      channels, chunksize=4)
    else:
        pool = mp.pool.ThreadPool(mp.cpu_count())
        channel_list[:] = pool.map(functools.partial(my_process_single_channel, particles, orig_pc,
                                                     beam_divergence, order, channel_infos), channels)
